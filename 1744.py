...
class Solution:
    def canEat(self, candiesCount: List[int], queries: List[List[int]]) -> List[bool]:
        ...
        for i in range(1, len(candiesCount)):
            arr.append(arr[i-1] + candiesCount[i-1])
        
        for i in range(len(queries)):
            if queries[i][1] < arr[queries[i][0]] + candiesCount[queries[i][0]] and arr[queries[i][0]] < (queries[i][1]+1)*queries[i][2]:
                result.append(True)
